...
p1 <- ggplot(data = wayne_idx_1990) +
  geom_sf(aes(fill = H)) +
  scale_fill_gradient(name = "Wskaźnik H", low = "white", high = "blue", limits = c(0, 1)) +  # Set legend title
  labs(title = "1990") + 
  theme_bw() + theme(
...
p2 <- ggplot(data = wayne_idx_2000) +
  geom_sf(aes(fill = H)) +
  scale_fill_gradient(name = "Wskaźnik H", low = "white", high = "blue", limits = c(0, 1)) +  # Consistent legend title
  labs(title = "2000") + 
  theme_bw() + theme(
    axis.text = element_blank(),
    axis.ticks = element_blank(),
